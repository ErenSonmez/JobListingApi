...
async def get_listing_page(page: int = 1, size: int = 10,
                           order_by: Annotated[list[str], Query()] = None,
                           listing_service: JobListingService = Depends(ServiceFactory.get_job_listing_service)) -> PaginatedResponse[JobListing]:
    print("router orderby", order_by)
    return await listing_service.get_page(page, size, order_by=order_by)
